...
SpeechDockは、macOSのTTS/STTをより柔軟で使いやすくします。macOSには強力な音声認識・音声合成エンジンが搭載されていますが、それらを便利に使う方法がありません。SpeechDockはこのギャップを埋めます — Macのどこからでもアクセスできるメニューバーアプリケーションです。
...
2. テキストエリアの下にある言語ボタン（例: `[🌐 Japanese ▼]`）をクリック
3. ターゲット言語を選択 — 翻訳がすぐに実行される
4. `[🌐 Original ◀]`をクリックして元のテキストに戻す

翻訳ボタンはテキストが3文字以上あり、録音/再生中でない場合に表示されます。
...
| タブ | オプション |
|------|----------|
| **一般** | プロバイダ、モデル、音声、言語、速度、パネルスタイル、ログイン時に起動 |
