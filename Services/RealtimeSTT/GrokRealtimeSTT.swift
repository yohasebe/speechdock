...
/// Grok Voice Agent API for real-time speech-to-text via WebSocket
/// Uses the OpenAI Realtime API compatible endpoint with transcription support
@MainActor
final class GrokRealtimeSTT: NSObject, RealtimeSTTService {
    ...
    var selectedLanguage: String = ""  // "" = Auto (Grok auto-detects)
    ...
    // Grok Voice Agent API supports 8kHz-48kHz, using 24kHz for compatibility with OpenAI
    ...
    // Track conversation item for proper accumulation
    // Within same item: transcript is cumulative (replace)
    // New item: transcript is new content (append)
    private var currentInputItemId: String = ""
    private var textBeforeCurrentItem: String = ""
    // Flag to ignore response items (not user input)
    private var isNextItemFromResponse: Bool = false
    ...
    func availableModels() -> [RealtimeSTTModelInfo] {
        [
            RealtimeSTTModelInfo(id: "grok-2-public", name: "Grok 2", description: "Grok Voice Agent with transcription", isDefault: true)
        ]
    }
    ...
    private func connectWebSocket(apiKey: String) async throws {
        // Grok Voice Agent API endpoint (OpenAI Realtime API compatible)
    ...
    private func configureSession() async throws {
        // Configure the session with transcription enabled
        // Grok uses server_vad for turn detection
        ...
        let config: [String: Any] = [
            "type": "session.update",
            "session": [
                "voice": "Ara",  // Required but not used for STT-only
                "audio": [
                    "input": [
                        "format": [
                            "type": "audio/pcm",
    ...
    private func parseMessage(_ jsonString: String) {
        ...
        case "conversation.item.added":
            ...
            // Skip if this is a response item (not user input)
            if isNextItemFromResponse {
                isNextItemFromResponse = false
            } else if let item = json["item"] as? [String: Any],
                      let itemId = item["id"] as? String {
                // This is a user input item - update tracking
                if currentInputItemId != itemId {
                    // New input item starting - save what we have so far
                    textBeforeCurrentItem = accumulatedText
        ...
        case "conversation.item.input_audio_transcription.delta":
            // Delta contains incremental text pieces (within current item, cumulative)
            if let delta = json["delta"] as? String, !delta.isEmpty {
                currentPartialText += delta
                // Display: text before this item + current item's cumulative partial
                let displayText = textBeforeCurrentItem + currentPartialText
                delegate?.realtimeSTT(self, didReceivePartialResult: displayText)
            }

        case "conversation.item.input_audio_transcription.completed":
            // Final transcription for current item (cumulative within item)
            if let transcript = json["transcript"] as? String, !transcript.isEmpty {
                // transcript is cumulative within this item
                // Combine with text from before this item started
                accumulatedText = textBeforeCurrentItem + transcript
                currentPartialText = ""
